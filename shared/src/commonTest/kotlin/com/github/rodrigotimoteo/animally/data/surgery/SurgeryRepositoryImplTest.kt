...
class SurgeryRepositoryImplTest {
    private lateinit var database: AnimallyDatabase
    private lateinit var sut: SurgeryRepositoryImpl

    @BeforeTest
    fun setup() {
        database = createTestDatabase()
        sut = SurgeryRepositoryImpl(database)
    }
    ...
    private fun newSurgery(
        ...
        Surgery(
            ...
            analgesia = "Flunixin",
            complications = "None",
            recoveryNotes = "Stall rest for 2 weeks",
            isActive = isActive,
    ...
    @Test
    fun `when patient has no surgeries then returns empty list`() {
        assertEquals(emptyList(), sut.getByPatient(999L))
    }

    @Test
    fun `when inserting then returns rows affected and retrieves by id`() {
        val id = sut.insert(newSurgery(patientId = 1L, date = LocalDate(2024, 5, 1)))
    ...
    fun `when getting by patient then returns list sorted by date descending`() {
        sut.insert(newSurgery(patientId = 1L, date = LocalDate(2024, 1, 10)))
        sut.insert(newSurgery(patientId = 1L, date = LocalDate(2024, 6, 20)))
        sut.insert(newSurgery(patientId = 1L, date = LocalDate(2024, 3, 15)))
    ...
    fun `when patient id differs then getByPatient returns only matching`() {
        sut.insert(newSurgery(patientId = 1L, date = LocalDate(2024, 5, 1)))
        sut.insert(newSurgery(patientId = 2L, date = LocalDate(2024, 5, 2)))
