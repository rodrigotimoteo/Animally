...
class ConsultationRepositoryImplTest {
    ...
    private lateinit var sut: ConsultationRepositoryImpl

    @BeforeTest
    fun setup() {
        database = createTestDatabase()
        sut = ConsultationRepositoryImpl(database)
    }
    ...
    private fun newConsultation(
        ...
        patientId: Long,
        date: LocalDate,
        subjective: String = "Owner reports lameness",
        objective: String = "Swelling noted",
        assessment: String = "Suspected tendonitis",
        plan: String = "Rest and NSAIDs",
    ...
    @Test
    fun `when patient has no consultations then returns empty list`() {
        assertEquals(emptyList(), sut.getByPatient(999L))
    }

    @Test
    fun `when inserting then returns rows affected and retrieves by id`() {
        val id = sut.insert(newConsultation(patientId = 1L, date = LocalDate(2024, 5, 1)))
    ...
    fun `when getting by patient then returns list sorted by date descending`() {
        sut.insert(newConsultation(patientId = 1L, date = LocalDate(2024, 1, 10)))
        sut.insert(newConsultation(patientId = 1L, date = LocalDate(2024, 6, 20)))
        sut.insert(newConsultation(patientId = 1L, date = LocalDate(2024, 3, 15)))
    ...
    fun `when updating then modifies existing fields`() {
        ...
        sut.update(
            newConsultation(
                ...
                assessment = "Tendonitis confirmed",
    ...
    fun `when patient id differs then getByPatient returns only matching`() {
        sut.insert(newConsultation(patientId = 1L, date = LocalDate(2024, 5, 1)))
        sut.insert(newConsultation(patientId = 2L, date = LocalDate(2024, 5, 2)))
