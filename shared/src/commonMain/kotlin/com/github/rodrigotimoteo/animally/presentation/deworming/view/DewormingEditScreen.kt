...
/**
 * Screen for creating or editing a deworming treatment record.
 *
 * @param viewModel The [DewormingEditViewModel] for this screen.
 * @param modifier Optional modifier.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun DewormingEditScreen(
    viewModel: DewormingEditViewModel,
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text(if (formState?.isEditing == true) "Edit Deworming" else "Add Deworming") },
                navigationIcon = {
...
@Composable
private fun DewormingEditForm(
    viewModel: DewormingEditViewModel,
    formState: DewormingFormState?,
    modifier: Modifier,
) {
    ...
    Column(modifier = columnModifier) {
        ...
        formState?.let { form ->
            DewormingMedicationFields(viewModel, form)
            DewormingScheduleFields(viewModel, form)
            Button(
...
@Composable
private fun DewormingMedicationFields(
    viewModel: DewormingEditViewModel,
    form: DewormingFormState,
) {
    OutlinedTextField(
        value = form.product,
        onValueChange = viewModel::onProductChange,
        label = { Text("Product *") },
        isError = form.productError != null,
        supportingText = { form.productError?.let { Text(it) } },
        singleLine = true,
...
@Composable
private fun DewormingScheduleFields(
    viewModel: DewormingEditViewModel,
    form: DewormingFormState,
) {
    OutlinedTextField(
